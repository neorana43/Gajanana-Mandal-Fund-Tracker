...
export default async function MandalDashboardPage({
  params,
}: {
  params: { slug: string };
}) {
  ...
  if (mandalError || !mandal) {
    return <div>Mandal not found.</div>;
  }

  const role = await getUserRole(supabase);

  // Scope all queries to mandal_id
  const [donationRes, expenseRes, sponsorRes, allocationRes] =
    await Promise.all([
      ...
      role === "admin"
        ? supabase.rpc("get_allocations_with_emails", { mandal_id: mandal.id })
  ...
  if (!hasData) {
    return (
      <div className="p-6 max-w-2xl w-full mx-auto text-center text-muted-foreground">
        <h1 className="text-xl font-bold mb-4">{mandal.name} Dashboard</h1>
        {mandal.logo_url && (
          <Image
            src={mandal.logo_url}
            alt="Mandal Logo"
